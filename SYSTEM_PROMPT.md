...
**Versão atual:** **v0.18** (deployada 2026-05-26 — inferência de objeção a partir do campo livre quando enum vazio).
...
| v0.15 | 2026-05-26 | Substituída por v0.16 — anti-cópia #6 | **3 patches de calibração após round test sistemático.** Round test executado em lead 23045915 com 54 disparos (3 perfis × 18 combinações). Bugs catalogados em `ROUND_TEST_RESULTS.md`. Patches: (1) Anti-omissão multi-objeção #1+institucional — força fala 4 com diluição quando QUALQUER financeira (#1, #2, #3, #10, #12) está marcada. (2) Anti-omissão #14 em multi-objeção — força fala dedicada com 3 perguntas sempre que #14 estiver na lista. (3) Datas-âncora trimestrais para #9 — Format Payload calcula `+30d/+90d/+180d/próximo ano letivo`. Validação inicial em 7 disparos reais (lead 29373445 com #2+#10) confirmou patch 1 funcionando. |

| v0.16 | 2026-05-26 | Substituída por v0.17 — Objeções (livre) | **Anti-cópia reforçada para #6.** Disparos reais (29404611, 29359951, 25502725, 23501144) revelaram cópia 100% literal das falas 2 e 3 do Exemplo 1 do prompt. Patch marca essas frases como "QUEIMADAS" e dá lista de variantes alternativas. Resultado parcial: fala 3 passou a alternar entre 2 variantes; fala 2 trocou a frase queimada por outra fixa (cópia deslocada, não eliminada). Decisão consciente de aceitar — cópia é cosmética, não funcional (vendedor adapta ao falar com cliente). Reaberto se aparecer em outros tipos de objeção. |

| v0.17 | 2026-05-26 | Substituída por v0.18 — inferência enum-vazio | **Uso ativo de `Objeções (livre)`.** Bug descoberto em lead 28231916 (Tatiana, Pública): campo "Objeções (livre)" = `"apoio da SME, porém informou que não tem verba nesse valor"` foi IGNORADO pelo LLM. Patch adiciona seção obrigando o roteiro a refletir a citação literal: fala 1 acolhe elementos específicos, fala 2 investiga pista revelada, fala 4 adapta ao gap. Validação imediata no mesmo lead. Não-regressão confirmada em 4 leads com #6. Validação adicional em lead 29483911 (3 objeções #1+#2+#14): patches v0.15 #1 (diluição em multi-objeção) e v0.15 #2 (3 perguntas #14 em combinação) ambos confirmados em produção real. |

| **v0.18 (ATIVA)** | **2026-05-26** | **DEPLOYADA** | **Inferência de objeção a partir do campo livre quando enum vazio.** Bug descoberto em lead 29479931 (Nelson, Pública): vendedor preencheu apenas `Objeções (livre)` = `"são 350 alunos, mesmo sendo um valor baixo, 80% ou dos estudantes não conseguem pagar este valor"` mas deixou enum vazio. Comportamento errado: `Format Payload` computava `ehObjecaoValida` baseado apenas no enum, então caía no branch defensivo "agente não pôde gerar roteiro" — citação rica ignorada totalmente. **2 mudanças:** (1) Format Payload — `livrePreenchido` agora é considerado como objeção válida (`ehObjecaoValida = objecoesReais.length > 0 \|\| livrePreenchido`). (2) System Prompt — nova seção "REGRA DE INFERÊNCIA quando enum vazio + livre preenchido" obrigando o LLM a inferir 1-3 objeções canônicas, aplicar Estratégia/Estruturas/Compromissos normalmente, e começar `por_que_funciona` com sinal explícito "Objeção inferida da citação livre: [#N nome]. Vendedor não marcou o enum — marcar próximo disparo." Validação imediata (exec 260486): output novo inferiu corretamente #10 Escola Pública + #2 SEM VERBA, gerou roteiro completo com cabeçalho "(nenhuma)" preservando rastreabilidade, e meta-info alertou vendedor a marcar o enum em disparos futuros. Risco baixo: comportamento idêntico ao anterior quando enum está preenchido (caminho padrão). |
