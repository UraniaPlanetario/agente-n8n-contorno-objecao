...
**Versão atual:** **v0.13.1** (deployada 2026-05-26 — cabeçalho clean "Contorno: <label literal Kommo>" sem disclaimer).
...
| **v0.13.1 (ATIVA)** | **2026-05-26** | **DEPLOYADA** | **Cabeçalho clean.** Antes: `OBJEÇÕES SETADAS NO DISPARO (snapshot — campo pode ter sido editado depois)` + `- Vou ver com a direção` em 2 linhas com bullet. Agora: `Contorno: Vou ver com a direção` em 1 linha. Label literal do Kommo mantido (auditável). Disclaimer "snapshot" removido (informação ainda fica implícita pela natureza da nota historicizada). v0.13 antes tinha tabela `LABELS_LIMPOS` mapeando labels Kommo pra versões "limpas" — Marcos preferiu manter label literal pra rastreabilidade. Aplicado nos 2 caminhos (Build Note + Build Orientation Note). |
