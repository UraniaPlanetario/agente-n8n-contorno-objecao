...
**Versão atual:** **v0.17** (deployada 2026-05-26 — uso ativo de `Objeções (livre)`).
...
| **v0.17 (ATIVA)** | **2026-05-26** | **DEPLOYADA** | **Uso ativo de `Objeções (livre)`.** Bug descoberto em lead 28231916 (Tatiana, Pública): campo "Objeções (livre)" = `"apoio da SME, porém informou que não tem verba nesse valor"` foi IGNORADO pelo LLM — output tratou como #2 SEM VERBA genérica. Causa: prompt não instrui o LLM a usar o campo como contexto prioritário. Patch adiciona seção "REGRA DE USO DO CAMPO `Objeções (livre)` (v0.17)" obrigando o roteiro a refletir a citação literal: fala 1 acolhe elementos específicos, fala 2 investiga pista revelada (aliança SME, tentativa anterior, gap parcial), fala 4 adapta ao gap (ex.: modalidade alternativa pra "verba mas não nesse valor"). Validação imediata no mesmo lead 28231916 (exec 260375): output novo menciona SME em todas as falas relevantes, propõe "modalidade menor" e "ajustar conforme SME". 4 leads com #6 isolado disparados em paralelo confirmaram não-regressão (tom humano, Estruturas, Compromissos todos preservados). |
